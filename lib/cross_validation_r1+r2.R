...
RMSE <- function(rating, est_rating){
  sqr_err_fun <- function(obs){
    sqr_error <- (obs[3] - est_rating[as.character(obs[2]), as.character(obs[1])])^2
    return(sqr_error)
  }
  return(sqrt(mean(apply(rating, 1, sqr_err_fun))))  
}


# Function that returns p, q, training RMSE, and testing RMSE.
r1_r2 <- function(f=10, lambda = 0.3,lrate = 0.01, max.iter, 
                        stopping.deriv = 0.01,data, train, test){
  set.seed(0)
  ...
  for(l in 1:max.iter){
    ...
    #loop iterate through each training case and perform update
...
# Reference https://github.com/TZstatsADS/fall2019-project4-sec2-grp1
